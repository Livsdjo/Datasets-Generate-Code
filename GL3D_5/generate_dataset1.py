...
"""
       5a2a95f032a1c655cfe3de62 dataset 生成代码
"""
def dataset_generate_main(config):
    # 显示一下图像的下标
    path = "E:\GL3D-2\github上传文件\GL3D_5\Input_Data/5a2a95f032a1c655cfe3de62_5/"
    print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    for i in trange(440):
        visualize_new_class_def_inst = visualize_new_class_def("./Input_Data/5a2a95f032a1c655cfe3de62_5/", i)
    ...
    for i in trange(440):
        ...
        if count == 440:
            ...
            train_data_path = os.path.join(config.datasets_output_path, "/COLMAP_hehe6/train")
            valid_data_path = os.path.join(config.datasets_output_path, "/COLMAP_hehe6/valid")
            test_data_path = os.path.join(config.datasets_output_path, "/COLMAP_hehe6/test")
