...
def dataset_generate_main(config):
    ...
    a = 0
    all_index = []
    for i in trange(4501):   # 4501  对于这个地方先保持怀疑吧  到时候再测一下吧
        visualize_new_class_def_inst = visualize_new_class_def("./Input_Data/5a533e8034d7582116e34209_5/", i)

        right_match_count = len(visualize_new_class_def_inst.match_records[visualize_new_class_def_inst.match_pair_idx][2][:, 0:6])
        # print("正确的个数:", right_match_count)
        # print("info", i, visualize_new_class_def_inst.basename0, visualize_new_class_def_inst.basename1)
        ...
        if visualize_new_class_def_inst.basename0 != a:
            if a == 0:
                ...
                all_index.append(info1[:, 0][:6].tolist())                              # 8
                all_index.append(info1[:, 0][len(info1) - 2:len(info1)].tolist())       # len(info1) - 12:len(info1):3
                # print(all_index)
                info.clear()
            a = visualize_new_class_def_inst.basename0
        info.append([i, right_match_count])

        if len(all_index) >= 227:
    ...
    for index, i in enumerate(tqdm(select_index_result)):
        ...
        if count == 900:
            ...
            train_data_path = os.path.join(config.datasets_output_path, "/COLMAP_hehe8/train")
            valid_data_path = os.path.join(config.datasets_output_path, "/COLMAP_hehe8/valid")
            test_data_path = os.path.join(config.datasets_output_path, "/COLMAP_hehe8/test")
