...
import numpy as np
import cv2
from utils.geom import get_essential_mat, get_epipolar_dist, undist_points, warp, grid_positions, upscale_positions, downscale_positions, relative_pose, evaluate_R_t
...
class MatcherWrapper(object):
    ...
    def get_matches(self, feat1, feat2, cv_kpts1, cv_kpts2, six_dem_kpts1, six_dem_kpts2, ratio=None, cross_check=True, err_thld=4, ransac=False, info=''):
        ...
        if type(cv_kpts1) is list and type(cv_kpts2) is list:
            ...
            other_info1 = np.array([cv_kpts1[m.queryIdx] for m in good_matches])
            other_info2 = np.array([cv_kpts2[m.trainIdx] for m in good_matches])
        ...
        # print("good kpts", good_kpts1, good_kpts2)

        # return good_matches, mask, [good_kpts1, good_kpts2], [good_kpts3, good_kpts4], [other_info1, other_info2]
