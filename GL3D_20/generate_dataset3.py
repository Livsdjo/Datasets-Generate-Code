...
class visualize_new_class_def:
    ...
    def get_error_match(self, mode, display=False):
...
"""
       588a34cfec4d5a1c088ea8d1  dataset 生成代码
"""
def dataset_generate_main(config):
    # 显示一下图像的下标
    """
    print("<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    for i in trange(440):
        visualize_new_class_def_inst = visualize_new_class_def("E:/data/5c0d13b795da9479e12e2ee9", i)
    ...
    # 4. 左右6维旋转信息
    # 5. 其他信息
    temp = visualize_new_class_def("./Input_Data/57102be2877e1421026358af_20/", 0)
    image_pair_count = len(temp.match_records)
    ...
    if 1:
        print("结束")
        # train_data_path = "E:/NM-Net-Initial/datasets/COLMAP_hehe_W/train"
        # valid_data_path = "E:/NM-Net-Initial/datasets/COLMAP_hehe_W/valid"
        # test_data_path = "E:/NM-Net-Initial/datasets/COLMAP_hehe_W/test"
        train_data_path = os.path.join(config.datasets_output_path, "/COLMAP_heheC/train")
        valid_data_path = os.path.join(config.datasets_output_path, "/COLMAP_heheC/valid")
        test_data_path = os.path.join(config.datasets_output_path, "/COLMAP_heheC/test")
